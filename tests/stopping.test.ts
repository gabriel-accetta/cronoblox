...
describe("bounded stopping rule", () => {
  ...
  it("stops visibly when a budget is reached", () => expect(evaluateStoppingRule({ requiredEvidencePresent: true, unresolvedHighSeverity: 1, furtherCallExpectedValue: "high", budgetReached: true })).toEqual({ stop: true, reason: "Configured budget reached; expose remaining gaps and reduce confidence" }));
