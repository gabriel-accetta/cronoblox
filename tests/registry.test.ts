...
describe("module registry", () => {
  it("locks Module 2 into every normal profile", () => { for (const id of ["baseline", "research-no-critic", "hackathon-full"]) expect(getProfile(id).enabled_modules[0]).toBe("roblox-data"); });
  ...
  it("uses explicit, unique module IDs", () => expect(new Set(registry.list().map((item) => item.manifest.id)).size).toBe(3));
});
