...
const worker = new Worker<{ runId: string }>("cronoblox-analysis", async (job) => { await executeRun(job.data.runId); }, { connection: redis, concurrency: Number(process.env.WORKER_CONCURRENCY ?? 2), lockDuration: 900_000 });
